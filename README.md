# Indian Quant Research Agent V2.6.1
...
## Important V2.6.1 fixes

- **Pattern scanning now starts automatically in parallel with deep confirmation** after Stage A screening. It runs server-side, so the browser/iPhone does not sit on a blocking spinner or need to remain awake. Pattern output is saved separately as `pattern_results.csv` and shown in the Pattern Scanner tab.
