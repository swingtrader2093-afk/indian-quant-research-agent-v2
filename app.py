...
def _save_job(job_dir,status,**extra):
    """Atomically-ish merge job status fields so parallel quant/pattern updates
    cannot erase each other's progress metadata."""
    status_path=job_dir/"status.json"
    old={}
    try:
        if status_path.exists(): old=json.loads(status_path.read_text())
    except Exception:
        old={}
...
def build_report(results,cfg,detail):
    ...
    L=["# Indian Quant Research Agent V2.6.1 — NSE Stock Screening, Quant Research & Parallel Pattern Scan Report",
...
if os.environ.get("QUANT_WORKER") != "1":
    ...
    st.title("🤖 Indian Quant Research Agent V2.6.1 — NSE Stock Screening, Quant Research & Parallel Pattern Scan")
        ...
        if status.get("status")=="complete":
            ...
            if payload:
                ...
                if not valid.empty:
                    ...
                    st.subheader("⭐ Ranked results"); st.caption("Research ranking is evidence strength, not a buy signal. WATCH means historical evidence deserves current-market review; SETUP is never assigned from backtest results alone.")
