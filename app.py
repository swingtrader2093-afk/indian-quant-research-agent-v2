...
def build_report(results,cfg,detail):
    ...
    L=["# Indian Quant Research Agent V2.6.4 — NSE Stock Screening, Quant Research & Parallel Pattern Scan Report",
...
if os.environ.get("QUANT_WORKER") != "1":
    ...
    st.title("🤖 Indian Quant Research Agent V2.6.4 — NSE Stock Screening, Quant Research & Parallel Pattern Scan")
    ...
    if st.button("🚀 Start Full Validation Agent",type="primary"):
        ...
        jid=launch_job(cfg); st.session_state["job_id"]=jid; st.success(f"Started job {jid}. Quant research is screening {len(universe):,} stocks and the current-pattern scan is running in parallel. Only the strongest candidates receive deep confirmation. You can lock the iPhone or close Chrome."); st.rerun()
    ...
    if job_id:
        ...
        if status.get("status")=="complete":
            ...
            if payload:
                ...
                st.download_button("⬇️ Download COMPLETE EVIDENCE BUNDLE",bundle,file_name="indian_quant_research_agent_v2_6_4_evidence.zip",mime="application/zip")
    ...
    st.markdown("---")
    st.caption("Research only. No order execution. No guarantee of profitability. V2.6.4 uses the current NSE equity universe for screening; this does not establish survivorship-bias-free historical results without point-in-time constituent/delisting data.")
