...
def true_walk_forward(df,bm,strategy,base,train_days=756,test_days=126):
    if len(df)<train_days+test_days+100: return None, pd.DataFrame(), pd.DataFrame()
...
def build_report(results,cfg,detail):
    valid=results[results.get("Verdict","").notna()].copy() if not results.empty else results
...
# ---------- UI ----------
st.title("🤖 Indian Quant Research Agent V2 — All-in-One")
...
if res is not None:
    valid=res[res.get("Verdict","").notna()].copy()
