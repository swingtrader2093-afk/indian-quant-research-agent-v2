...
st.set_page_config(page_title="Indian Quant Research Agent V2.8", page_icon="🤖", layout="wide")
        ...
        for ticker in tickers:
            ...
            for strat in strategies:
                try:
                    ...
                    current_rs60=float(last.get("rs60",np.nan)) if "rs60" in last else np.nan
...
def build_report(results,cfg,detail):
    ...
    L=["# Indian Quant Research Agent V2.8 — Master Shortlist + NSE Quant Research + Parallel Pattern Scan Report",
...
def build_master_shortlist(res, pattern_df):
    ...
    if m.empty:
        return m, r
    # Practical status: strict Master requires volume confirmation; pattern
    # setups without confirmed volume remain visible as Setup Watch.
    ...
    # Prefer true MASTER rows, then stronger research/pattern evidence.
    m=m.sort_values(["Master status","Research score","Pattern score","WF Sharpe"],ascending=[True,False,False,False])
    return m,r
...
if os.environ.get("QUANT_WORKER") != "1":
    ...
    st.title("🤖 Indian Quant Research Agent V2.8 — Master Shortlist + NSE Quant Research + Parallel Pattern Scan")
    st.caption("Broad NSE screening → automatic candidate selection → expensive confirmation, with an independent current-pattern scan running in parallel during the deep-research stage. Includes Markov + HMM + GARCH-style volatility + Monte Carlo + train-only walk-forward + sensitivity + research memory. Long runs continue in a server-side worker so the iPhone screen can sleep.")
    ...
    with tab_master:
        ...
        st.caption("The daily decision view: the SAME strategy must have strong historical evidence, be active on the latest bar, and have a current chart pattern. MASTER additionally requires volume confirmation.")
        st.info("Run the full agent from this tab. The underlying Markov, HMM, GARCH-style volatility, Monte Carlo, 8 strategies, walk-forward, sensitivity and pattern engines remain unchanged.")
        st.markdown("**Flow:** 2,000 stocks → current strategy signal → historical validation → walk-forward/robustness → current pattern → volume → ⭐ Master Shortlist → chart review")
        ...
        st.caption("Once a run completes, this tab shows only candidates satisfying the master gates. It is not a buy signal.")
        ...
        if status.get("status")=="complete":
            ...
            if payload:
                ...
                if not valid.empty:
                    ...
                    st.subheader("⭐ Master shortlist")
                    if master.empty:
                        st.info("No strict Master candidates in this run: the same strategy must be historically PROMISING, active on the latest bar, and have a current pattern. Volume-confirmed MASTER rows are preferred; setup-watch rows remain visible in the Opportunity tab.")
                    else:
                        ...
                        st.caption("MASTER = PROMISING historical evidence + SAME strategy active now + current pattern + volume PASS. SETUP WATCH = same gates but pattern volume is not yet PASS. Review the chart and apply your own entry/stop/position-sizing rules before trading.")
                ...
                st.download_button("⬇️ Download COMPLETE EVIDENCE BUNDLE",bundle,file_name="indian_quant_research_agent_v2_8_evidence.zip",mime="application/zip")
    ...
    st.markdown("---")
    st.caption("Research only. No order execution. No guarantee of profitability. V2.7 uses the current NSE equity universe for screening; this does not establish survivorship-bias-free historical results without point-in-time constituent/delisting data.")
