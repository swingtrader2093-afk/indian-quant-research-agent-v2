...
def true_walk_forward(df,bm,strategy,base,train_days=756,test_days=126):
    ...
    while pos<len(df):
        ...
        cand=best[1]
        # Use all data up to the end of the test fold for indicator warm-up,\n        # but permit trades only from the test-fold start onward.\n        test_context=df.iloc[:test_end]\n        tc,ec,m=backtest(test_context,bm,strategy,cand,capital,start=test.index[0],end=test.index[-1])
        if not ec.empty:
                ...
                try:
                    ...
                    base={
                        ...
                        **quant,
                        ...
                        "Bias checks":"REVIEW — survivorship universe"
                    }
